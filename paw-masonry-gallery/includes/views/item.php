...
<div class="<?php echo esc_attr(implode(' ', $item_classes)); ?>">
    ...
    <div class="dmg-item-content">
        ...
        <?php if ('no-link' !== $image_data['click_action']) : ?>
            <a <?php echo wp_kses_data(Divi_Masonry_Gallery\Helper::render_attributes($link_attributes)); ?> class="<?php echo esc_attr(implode(' ', $link_classes)); ?>" /></a>
        <?php endif ?>
        <div class="dmg-caption">
            <div class="dmg-caption-inner">
                <?php if ('on' !== $this->props['hide_caption'] && 'on' !== $this->props['hide_title']) : ?>
                    <h2 class='dmg-title'><?php echo wp_kses_post($image_data['title']); ?></h2>
                <?php endif ?>
                <?php if ('on' !== $this->props['hide_caption'] && 'on' !== $this->props['hide_description']) : ?>
                    <p class="dmg-description"><?php echo wp_kses_post($image_data['description']); ?></p>
                <?php endif ?>
            </div>
        </div>
    </div>
